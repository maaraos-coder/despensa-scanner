...
CATALOG_ID = "catalogo_actual_la_despensa"
...
def supabase_catalog_table():
    return st.secrets.get("SUPABASE_CATALOG_TABLE", "catalogo_actual")
...
def serialize_catalog_df(df):
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        return []
    safe = df[ELEVENTA_COLUMNS].copy()
    rows = []
    for _, row in safe.iterrows():
        rows.append(row_to_dict(row))
    return rows

def restore_catalog_df(rows):
    ...
    for d in rows:
        if not isinstance(d, dict):
            continue
        fixed = {col: d.get(col, "" if col not in NUMERIC_COLUMNS else 0) for col in ELEVENTA_COLUMNS}
        fixed["Código"] = normalize_code(fixed["Código"])
        for col in NUMERIC_COLUMNS:
            fixed[col] = clean_money(fixed[col])
        fixed_rows.append(fixed)
    if not fixed_rows:
        return None
    return ensure_eleventa_columns(pd.DataFrame(fixed_rows))

def save_catalog_to_supabase(df, filename="catalogo_eleventa.xlsx", show_error=False):
    if not supabase_ready():
        if show_error:
            st.warning("Supabase no está configurado en Secrets.")
        return False
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        if show_error:
            st.warning("No hay catálogo para guardar.")
        return False

    url, key, _ = supabase_config()
    table = supabase_catalog_table()
    endpoint = f"{url}/rest/v1/{table}?on_conflict=id"
    payload = {
        "id": CATALOG_ID,
        "nombre_archivo": str(filename or "catalogo_eleventa.xlsx"),
        "data": {
            "rows": serialize_catalog_df(df),
            "total": int(len(df)),
            "updated_from_app": datetime.now().isoformat(timespec="seconds"),
        },
        "updated_at": datetime.now().isoformat(timespec="seconds"),
    ...
    try:
        ...
        ok = r.status_code in (200, 201, 204)
        if not ok and show_error:
            st.error(f"Supabase no guardó el catálogo. HTTP {r.status_code}: {r.text[:500]}")
        return ok
    except Exception as e:
        if show_error:
            st.error(f"Error guardando catálogo en Supabase: {e}")
        return False

def load_catalog_from_supabase(show_error=False):
    if not supabase_ready():
        return False
    url, key, _ = supabase_config()
    table = supabase_catalog_table()
    endpoint = f"{url}/rest/v1/{table}?id=eq.{CATALOG_ID}&select=nombre_archivo,data,updated_at"
    try:
        r = requests.get(endpoint, headers=supabase_headers(), timeout=30)
        if r.status_code != 200:
            if show_error:
                st.error(f"No pude leer catálogo desde Supabase. HTTP {r.status_code}: {r.text[:500]}")
            return False
        rows = r.json()
        if not rows:
            return False
        ...
        df = restore_catalog_df(data.get("rows", []))
        if df is None:
            return False
        st.session_state.catalog_df = df
        st.session_state.catalog_index = build_index(df)
        st.session_state.catalog_filename = item.get("nombre_archivo", "catálogo guardado")
        st.session_state.catalog_updated_at = item.get("updated_at", "")
        return True
    except Exception as e:
        if show_error:
            st.error(f"Error leyendo catálogo desde Supabase: {e}")
        return False

def clear_catalog_supabase(show_error=False):
    if not supabase_ready():
        return False
    url, key, _ = supabase_config()
    table = supabase_catalog_table()
    endpoint = f"{url}/rest/v1/{table}?id=eq.{CATALOG_ID}"
    try:
        r = requests.delete(endpoint, headers=supabase_headers("return=minimal"), timeout=12)
        ok = r.status_code in (200, 202, 204)
        if not ok and show_error:
            st.error(f"No pude borrar catálogo. HTTP {r.status_code}: {r.text[:500]}")
        return ok
    except Exception as e:
        if show_error:
            st.error(f"Error borrando catálogo: {e}")
        return False
...
if "catalog_loaded_from_supabase" not in st.session_state:
    st.session_state.catalog_loaded_from_supabase = False
if "catalog_filename" not in st.session_state:
    st.session_state.catalog_filename = ""
...
if not st.session_state.catalog_loaded_from_supabase and st.session_state.catalog_df is None:
    load_catalog_from_supabase(show_error=False)
    st.session_state.catalog_loaded_from_supabase = True
...
with st.expander("☁️ Estado Supabase", expanded=False):
    if supabase_ready():
        ...
        with cload:
            if st.button("Recargar borrador", use_container_width=True):
        ...
        with ccat1:
            if st.button("Recargar catálogo", use_container_width=True):
                if load_catalog_from_supabase(show_error=True):
                    st.success("Catálogo recuperado desde Supabase.")
                    st.rerun()
                else:
                    st.warning("No encontré catálogo guardado.")
        with ccat2:
            if st.button("Guardar catálogo actual", use_container_width=True):
                if save_catalog_to_supabase(st.session_state.catalog_df, st.session_state.get("catalog_filename", "catalogo_eleventa.xlsx"), show_error=True):
                    st.success("Catálogo guardado en Supabase.")
    else:
...
with st.expander("📂 Catálogo Eleventa", expanded=st.session_state.catalog_df is None):
    if st.session_state.catalog_df is not None:
        ...
        if st.session_state.get("catalog_filename"):
            st.caption(f"Archivo: {st.session_state.catalog_filename}")
        if st.session_state.get("catalog_updated_at"):
            st.caption(f"Guardado en Supabase: {st.session_state.catalog_updated_at}")
        st.caption("Si subes un nuevo Excel, reemplazará el catálogo guardado del día.")

    uploaded = st.file_uploader("Sube o reemplaza el Excel exportado desde Eleventa", type=["xlsx"])
    ...
    if uploaded is not None:
        try:
            ...
            st.session_state.catalog_filename = uploaded.name
            st.session_state.catalog_updated_at = datetime.now().isoformat(timespec="seconds")
            ok = save_catalog_to_supabase(st.session_state.catalog_df, uploaded.name, show_error=True)
            st.success(f"Catálogo cargado: {len(st.session_state.catalog_df):,} productos".replace(",", "."))
            if ok:
                st.success("Catálogo guardado en Supabase. No tendrás que subirlo de nuevo si cierras la página.")
        except Exception as e:
            st.error(f"No pude leer el catálogo: {e}")
    elif st.session_state.catalog_df is None:
        st.info("Primero sube el catálogo exportado de Eleventa. Luego quedará guardado en Supabase para este día.")

if st.session_state.catalog_df is not None:
    msg = f"✅ Catálogo activo: {len(st.session_state.catalog_df):,} productos".replace(",", ".")
    if st.session_state.get("catalog_filename"):
        msg += f" · {st.session_state.catalog_filename}"
    st.success(msg)
...
with st.expander("ℹ️ Cómo importar en Eleventa"):
    st.markdown(
        ...
        El catálogo diario queda guardado en Supabase. Si al día siguiente exportas uno nuevo desde Eleventa/Drive, súbelo en **Catálogo Eleventa** para reemplazar el anterior.
        """
    )
