...
def clean_code(x) -> str:
    """Limpia un código leído desde cámara/Excel sin perder ceros iniciales si venía como texto."""
    if pd.isna(x):
        return ""
    s = str(x).strip()
    # Excel a veces transforma códigos largos a número o notación científica.
    try:
        if "e" in s.lower():
            s = f"{float(s):.0f}"
    except Exception:
...
def code_key(x) -> str:
    """Clave flexible para búsqueda: elimina ceros iniciales y normaliza formatos numéricos."""
    c = clean_code(x)
    if not c:
        return ""
    if c.isdigit():
...
def code_variants(x) -> List[str]:
    ...
    add(c)
    if c.isdigit():
        # Sin ceros iniciales: 079919222573 == 79919222573
        add(c.lstrip("0") or "0")
        # Con ceros iniciales típicos UPC/EAN por si el Excel o el lector los agrega/quita
        for length in (8, 12, 13, 14):
            if len(c) < length:
                add(c.zfill(length))
            stripped = c.lstrip("0") or "0"
            if len(stripped) < length:
                add(stripped.zfill(length))
        # A veces el lector antepone o pierde un 0 inicial.
...
if st.session_state.last_code:
    st.caption("Búsqueda flexible activa: también busca sin ceros iniciales y con formatos UPC/EAN.")
...
def find_product(code: str):
    ...
    if df is None or not code or not cols.get("codigo"):
        return None
    tmp = df.copy()
    code_col = cols["codigo"]
    target_variants = set(code_variants(code))
    target_keys = set(code_key(v) for v in target_variants if v)
    tmp["__code_clean__"] = tmp[code_col].apply(clean_code)
    tmp["__code_key__"] = tmp[code_col].apply(code_key)

    # 1) coincidencia exacta
    matches = tmp[tmp["__code_clean__"].isin(target_variants)]
    if not matches.empty:
        return matches.iloc[0]

    # 2) coincidencia flexible sin ceros iniciales
    matches = tmp[tmp["__code_key__"].isin(target_keys)]
    if not matches.empty:
        return matches.iloc[0]

    return None

def catalog_code_from_row(row) -> str:
    cols = st.session_state.cols
    if row is None or not cols.get("codigo"):
        return clean_code(st.session_state.last_code)
    return clean_code(row.get(cols.get("codigo"), st.session_state.last_code))
...
if code:
    ...
    if is_new:
        ...
        with st.expander("Ver cómo se buscó el código", expanded=False):
            st.write("La app buscó estas variantes para evitar fallas por ceros iniciales:")
            st.code(", ".join(code_variants(code)))
        desc_default = ""
        stock_default = 0.0
        cost_default = 0.0
        price_default = 0.0
        ...
        matched_catalog_code = catalog_code_from_row(row)
        if matched_catalog_code != code:
            st.success(f"Producto encontrado: {desc_default} — coincidió con código del catálogo {matched_catalog_code}")
        else:
    ...
    if submitted:
        export_code = code if is_new else catalog_code_from_row(row)
        item = {
            "codigo": export_code,
            "codigo_leido": code,
            "descripcion": desc,
